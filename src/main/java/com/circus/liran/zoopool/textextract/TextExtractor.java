package com.circus.liran.zoopool.textextract;
...
public class TextExtractor {
	...
	public void extractURL(String url) throws IOException {
		...
		if (isLegalURL(url)) {
			if (isContentURL(url)) {
				...
				conn.timeout(50000);
				String htmlText = conn.get().toString();
	...
	private List<Integer> lineBlockDistribute(List<String> lines) {
		...
		for (int i = 0; i + 4 < lines.size(); i++) {
			if (indexDistribution.get(i) == 0 && indexDistribution.get(i + 1) == 0 && indexDistribution.get(i + 2) > 0 && indexDistribution.get(i + 2) < 40
	...
	public static void main(String[] args) throws IOException {
		...
		te.extractURL("http://live.kankanews.com/cehua/2013-09-04/0013464941_62.shtml");
